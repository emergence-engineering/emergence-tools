...
export const codeMirrorBlockNodeView: (
  ...
  const createCopyButtonCB = settings.createCopyButton(
    settings,
    dom,
    node,
    view,
  ...
  setTheme(codeMirrorView, themeConfig, [
  ...
  themeCallbacks.push(updateTheme);
  ...
  return {
    ...
    destroy: () => {
      selectDeleteCB();
      createCopyButtonCB();
      themeCallbacks.splice(themeCallbacks.indexOf(updateTheme), 1);
    },
  };
};
